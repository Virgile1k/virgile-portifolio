...
interface Props {
  blog: Blog & { _id: string }; 
}

export default function BlogCard({ blog }: Props) {

  const publishedDate = blog.publishedDate instanceof Date ? blog.publishedDate.toDateString() : new Date(blog.publishedDate).toDateString();

  return (
    <div className="relative bg-blue-100 p-4 rounded-lg max-w-md">
      <img src={blog.blogImage} alt={blog.blogTitle} className="w-full h-48 object-cover rounded-lg" />
